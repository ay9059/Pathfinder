...
public class BoardGraph implements Graph< Dot > {
    ...
    public Iterable< Dot > getNodes() {
        List<Dot> temp = new ArrayList<>();
        for(Coordinate c : nodeMap.keySet()){
            temp.add(nodeMap.get(c));
        }
        return temp;
...
//        };
     ...
     */
    @Override
    public Dot makeNode( Coordinate name , int team, int weight) {
        Dot result = new Dot( name , team, weight);
    ...
    /**
     * Returns the dimension of the board, used in allStar class to check if the user has reached the final
     * row or column
     * @return an integer value representing the dimension of the board.
     */
    public int getDim(){
        return this.dim;
    }
